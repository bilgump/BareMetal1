...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define NTASKS (sizeof(tasks)/sizeof(tasks[0]))
/* USER CODE END PD */
	...
	while (1)
	{
		...
		for(uint8_t i = 0; i < NTASKS; ++i){
			if((now - tasks[i].lastMS) >= tasks[i].periodMS){
				tasks[i].lastMS = now;
				tasks[i].task();		//Function pointer acts like a jumpTo
			}
		}
...
//	{
//		HAL_GPIO_TogglePin(GPIOA, LED_1_Pin);   // flip LED
//	}
}
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
	if(hadc->Instance == ADC1){
		...
		temperature  = ((V25 - Vsense) / AvgSlope) + 25.0f;
...
void HAL_GPIO_EXTI_Callback(uint16_t pin)
{
	if(pin == GPIO_PIN_3) //PE3
		sleepRequested = !sleepRequested;	//flip flag
}
